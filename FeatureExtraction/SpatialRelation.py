...
def pt_to_nearest_geoObj(grid,shpfile):
    """
    grid points to their nearest spatial geometry objects (from shapefile)  
    """
    gpd_pts = gpd.GeoSeries(geom.Point(x,y) for x, y in grid)
    whereMinDist = np.zeros(len(grid))    
    for i in xrange(len(whereMinDist)):
        whereMinDist[i] = shpfile.distance(gpd_pts[i]).idxmin()
...
#        stat_fun = lambda x: x
    ...
    for i, grd_pt in enumerate(grid):
        ...
        if all(isInPoly[i,:]==False) or (stat_fun is None):
            stat[i,:] = np.nan
        else:
