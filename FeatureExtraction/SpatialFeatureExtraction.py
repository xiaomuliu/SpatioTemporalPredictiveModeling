...
def extract_proximity(grid,fields,shpfilenames):          
    ...
    for i, filename in enumerate(shpfilenames):
        ...
        minDist[:,i] = pt2geoObj_dist(grd,shpfile)
    return minDist    
    ...
    for i, filename in enumerate(shpfilenames):
        ...
        for j, patch_sz in enumerate(patchsizes): 
            if all(shpfile.geometry.geom_type == 'Point') or all(shpfile.geometry.geom_type =='Polygon'):
                spAttrDen[:,len(patchsizes)*i+j] = pt_within_poly_stat(grd,shpfile,patch_sz,stat_fun=np.nansum,proj=proj_str)
            elif all(shpfile.geometry.geom_type == 'LineString'):
                spAttrDen[:,len(patchsizes)*i+j] = line_intersect_poly_stat(grd,shpfile,patch_sz,stat_fun=np.nansum,proj=proj_str) 
    return spAttrDen            
...
def extract_bldg_attr(grid,fields,shpfilenames,patchsizes=None,proj_str=None):
    ...
    for i, attr in enumerate(fields):
        ...
        for j, patch_sz in enumerate(patchsizes):    
            bldgAttr[:,len(patchsizes)*i+j] = pt_within_poly_stat(grd,shpfile_sub,patch_sz,field=field,stat_fun=statFun,proj=proj_str)     
...
if __name__=='__main__':
    ...
    param_match = re.findall('\d+', params)
