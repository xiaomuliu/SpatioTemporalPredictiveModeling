...
path_featurefile="../SharedData/FeatureData/grid_${cellsize[0]}_${cellsize[1]}/Seq/Tr12_13_Te14/"
path_modelfile="../SharedData/ModelData/grid_${cellsize[0]}_${cellsize[1]}/Seq/Tr12_13_Te14/"
...
loadfile_train="${path_featurefile}feature_target_seq_train_dataframe.h5"
loadfile_test=$( ls ${path_featurefile}feature_target_seq_test_*_dataframe.h5)
...
python FitSeqPredModel.py -i "group=${loadfile_group} traindata=${loadfile_train} testdata=${loadfile_test} cluster=${loadfile_cluster} district=${loadfile_district}" -o ${savepath} -p "targetcrime=${target_crime} model=${pred_model} kfolds=${param_repro[0]} rseed=${param_repro[1]} trainregion=${train_region} trainregionNo=${train_region_num} testregion=${test_region} testregionNo=${test_region_num}" > "${savepath}experiment_log.txt"
