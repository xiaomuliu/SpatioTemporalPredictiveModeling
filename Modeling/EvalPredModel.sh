...
do
	...
	then
        ...
        path_modelfile="../SharedData/ModelData/grid_${cellsize[0]}_${cellsize[1]}/Tr12_13_Te14/${target_crime}/"
        ...
        loadfile_group="../SharedData/FeatureData/grid_${cellsize[0]}_${cellsize[1]}/Tr12_13_Te14/train_test_groups.pkl"
...
#notice the change
model_names=$(<./model_names.txt)

savepath="./Evaluation/grid_${cellsize[0]}_${cellsize[1]}/${target_crime}/Tr12_13_Te14/Thesis/${eval_region}${eval_region_num}/"

mkdir -p $savepath

python EvalPredModel.py -i "grid=${loadfile_grid} crime=${loadfile_crime} group=${loadfile_group} baseline=${loadfile_baseline} predscore=${loadfile_predscore[@]} samplemask=${loadfile_samplemask[@]} cluster=${loadfile_cluster} clustermask=${loadfile_clustermask} district=${loadfile_district} districtmask=${loadfile_districtmask}" -o ${savepath} -p "targetcrime=${target_crime} evalregion=${eval_region} evalregionNo=${eval_region_num} model=${model_names} evalspec=${param_eval}" > "${savepath}p_vals.txt"
