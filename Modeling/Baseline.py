...
#    from SetupGrid import flattened_to_geoIm 
    ...
    param_match4 = re.search('(?<=targetcrimetypes=)([A-Za-z_\s]+)(?=chunk)', params)
    param_match5 = re.search('(?<=chunk=)(\d+) (\d+)', params)
    ...
    chunksize_test = int(param_match5.group(2)) 
    
    # load crime data
    CrimeTypes = re.findall('[A-Za-z_]+',param_match4.group(0))
    CrimeData = []              
    ...
    for i in range(groups_test[0],groups_test[-1], chunksize_test):
        ...
        baseline_test = structure_baseline_model(CrimeData, CrimeTypes, groups_test_chunk, grid_2d, gauss_filter, \
...
#        plot_baseline(example_LT,example_ST)
        ...
        savefile_df = filePath_save+'baseline_chunk'+str(chunk_No)+'_dataframe.pkl'
        with open(savefile_df,'wb') as output: 
            pickle.dump(baseline_df, output, pickle.HIGHEST_PROTOCOL)
        
        baseline_test_stacked.append(baseline_test)    
        chunk_No += 1
   
    # save stacked baseline scores 
    baseline_test_stacked = np.vstack(baseline_test_stacked)    
    baseline_test_stacked_df = pd.DataFrame(baseline_test_stacked,columns=model_names)
    savefile_df = filePath_save+'baseline_dataframe.pkl'
    with open(savefile_df,'wb') as output: 
        pickle.dump(baseline_test_stacked_df, output, pickle.HIGHEST_PROTOCOL)                 